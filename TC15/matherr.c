...
Description	When  exceptions are  detected in  the math  library then a
		...
		The  default version  of Turbo	C's matherr  routine simply
		returns 0;  it serves as a  hook that you can  replace when
...
Return value	The default return  value for matherr is simply  0. matherr
		can  also   modify  e->retval,	which	propagates  through
