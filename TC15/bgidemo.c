/*
 ...
 From the Integrated environment, use bgidemo.prj.
*/
...
void ReportStatus(void)
{
 ...
 driver = DriverNames[ GraphDriver ];
 mode = GetMode();			/* get current setting		*/
...
void PutImageDemo(void)
{
 ...
 floodfill(StartX+1, StartY+4, GREEN);
...
/*	PALETTEDEMO: Change the palette randomly until the user says    */
/*      enough.					                        */
...
/*	GETMODE: Convert the current device mode into a string for	*/
/*	displaying in status.						*/

char *GetMode(void)
{
 static char buffer[40];

 switch( GraphDriver ){		/* Determine the mode setting	*/
 case CGA:
   if( GraphMode == CGAHI ) return( "CGAHI" );
   else{
     sprintf( buffer, "CGA%d", GraphMode );
     return( buffer );
   }
  case MCGA:
   switch( GraphMode ){
   ...
   case MCGAHI:
     return( "MCGAHi"	);   
   default:
     sprintf( buffer, "MCGA%d", GraphMode );
     return( buffer );
   }				/* End of GraphMode switch	*/
 ...
 case ATT400:
   ...
   case ATT400HI:
     return( "ATT400Hi"  );  
   default:
     sprintf( buffer, "ATT400%d", GraphMode );
     return( buffer );
   }				/* End of GraphMode switch	*/
 ...
 }					/* End of GraphDriver switch	*/

 return( "Unknown" );			/* Return as a default value	*/
...
void Pause(void)
{
 ...
 if( ESC == getch() ){			/* User wishes to leave		*/
