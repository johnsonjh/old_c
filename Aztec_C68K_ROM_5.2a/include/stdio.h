...
#define _IOSYNC 	0x2000	/* MPW compatibility, 
							 * input triggers fflush() to output fp's.
							 * will synchronize input and output fp's, such that
 							 * a read() for an input fp with this bit set will 
							 * first fflush() all output fp's which have the 
							 * _IOSYNC bit set.
  							 */
...
int _filbuf(FILE *);
void _getbuf(FILE *);
FILE *_fileopen(const char *, const char *, FILE *fp, int);
FILE *_getiob(void);
int _format(void *, int (*)(int, void *), const char *, va_list);
int format(int (*)(int), const char *, ...);
int _scan(void *, int (*)(int, void*, int), const char *, va_list);
