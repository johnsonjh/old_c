...
*
*   Sierra Systems C Run Time (CRT) header, 167bug version
*
...
;   jmp	    __xcptn_info_040; default 68040 exception handler

startup:
    cinva   bc		    ; invalidate both instruction and data caches
.ifdef COPYBACK
    move.l  #0xc020,d0
    movec   d0,dtt0	    ; enable data cache copyback mode
.endif
    move.l  #0x80008000,d0
    movec   d0,cacr	    ; enable both caches
    rts

cache_off:
    moveq   #0,d0
.ifdef COPYBACK
    cpusha  bc		    ; push data to memory and invalidate both caches
.else
    cinva   bc		    ; invalidate both instruction and data caches
.endif
    movec   d0,cacr	    ; disable both cache
    rts

monitor:
    move.l  (sp)+,d0	    ; put return value from _exit into d0
    trap    #15
    .word   0x63	    ; return to 167bug
