...
static struct TextAttr MyFont = {(UBYTE *)"topaz.font",TOPAZ_EIGHTY,FS_NORMAL,FPF_ROMFONT };
...
static struct StringInfo osx = { NL ,(UBYTE *)ubuf, NL,DSIZE,NL,NL,NL,NL,NL,NL,NL,NL,NL};
...
static struct StringInfo os5 = { NL ,(UBYTE *)ubuf, NL,DIR_SIZ,NL,NL,NL,NL,NL,NL,NL,NL,NL};
...
char *get_fname(cW,screen,hail,ddef,ddir)
   ...
   osx.Buffer = (UBYTE *)ddef;	/* Set default file name   */
   os5.Buffer = (UBYTE *)ddir;	/* Set default device name   */
   ...
   if ( pnam = (TEXT *)dinit(ddir) )   notify(pnam);
   ...
   for ( retval= NL, keepon=TRUE; keepon ; )
      ...
      while ( ! (imes=(struct IntuiMessage *)GetMsg(wRq->UserPort)) )
      {
         ...
         if ( more )
            {
            if (pnam = (TEXT *)dmore())  /* Continue to read the directory */
...
static char *dinit(subdir)
   ...
   if (! (pdir=(struct FileLock *)Lock((UBYTE *)subdir,(ULONG)ACCESS_READ)) )
      return("Wrong Diskette?");
   if ( ! Examine((BPTR)pdir, dir_info) )   return("Wierd Disk Error");
...
static cxxx()
   ...
   for (i=0; i<DENTS; i++)
      ...
      if ( (new+i) < maxent )
         {
         ohboy = ohboy->next;
         rname[i].IText = (UBYTE *)&ohboy->dE[0];
