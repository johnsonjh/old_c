...
/* #includes figured out by MWS 8-) -- Manx Software Systems 1/9/90 */
/* Also made other (minor) changes to keep function prototypes happy */
...
OpenStuff () {
        ...
        if ((Manmap_chip[0] = AllocMem((LONG)sizeof(Manmap[0]),MEMF_CHIP)) 
				== NULL)
             CloseStuff (1);
        if ((Manmap_chip[1] = AllocMem((LONG)sizeof(Manmap[1]),MEMF_CHIP)) 
				== NULL)
             CloseStuff (1);
        ...
        BltTemplate((char *)Manmap_chip[0],0L,12L,win->RPort,0L,0L,88L,88L);
        SetAPen(win->RPort, 2L);
       SetWrMsk(win->RPort, 2L);       /* Braindead COMPLEMENT */
        BltTemplate((char *)Manmap_chip[1],0L,12L,win->RPort,0L,0L,88L,88L);
        SetWrMsk(win->RPort, 255L);     /* Braindead COMPLEMENT */
...
int exitcode; 
{
        ...
        if (Eyemap_chip)
			FreeMem(Eyemap_chip, (LONG)sizeof(Eyemap));
        if (Manmap_chip[0])
			FreeMem(Manmap_chip[0], (LONG)sizeof(Manmap[0]));
        if (Manmap_chip[1])
			FreeMem(Manmap_chip[1], (LONG)sizeof(Manmap[1]));
        exit (exitcode);
}
...
int idx, OffX, OffY, col;
{
        ...
        BltTemplate((char *)Eyemap_chip, 0L, 2L, win->RPort,
